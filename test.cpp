...
BOOST_AUTO_TEST_CASE(InitiliaseParticle) { 
    ...
    BOOST_CHECK_CLOSE(p.get_r()[0], 10, tol);
    BOOST_CHECK_CLOSE(p.get_r()[1], 10, tol);
    BOOST_CHECK_CLOSE(p.get_r()[2], 10, tol);

    BOOST_CHECK_CLOSE(p.get_v()[0], 1, tol);
    BOOST_CHECK_CLOSE(p.get_v()[1], 1, tol);
    BOOST_CHECK_CLOSE(p.get_v()[2], 1, tol);

    BOOST_CHECK_CLOSE(p.get_F()[0], 0, tol);
    BOOST_CHECK_CLOSE(p.get_F()[1], 0, tol);
    BOOST_CHECK_CLOSE(p.get_F()[2], 0, tol);

    BOOST_CHECK_EQUAL(p.get_type(), 0);
    BOOST_CHECK_CLOSE(p.get_mass(), 1.0, tol);
}
...
BOOST_AUTO_TEST_CASE(ParticlePositionUpdate) {
    ...
    BOOST_CHECK_CLOSE(p.get_r()[0], 10.005, tol);
    BOOST_CHECK_CLOSE(p.get_r()[1], 10.002, tol);
    BOOST_CHECK_CLOSE(p.get_r()[2], 10.001, tol);
}
...
BOOST_AUTO_TEST_CASE(ParticleBoundaryCondition) {
    ...
    BOOST_CHECK_CLOSE(p.get_r()[0], 19.6, tol);
    BOOST_CHECK_CLOSE(p.get_r()[1], 10, tol);
    BOOST_CHECK_CLOSE(p.get_r()[2], 10, tol);
